...
export async function POST(req: NextRequest) {
  const { id, when } = await req.json().catch(() => ({}))
  ...
  const smsBody = es
    ? `Hola ${c.full_name}, tu entrevista con BSM Facility Solutions está programada para ${when}. Responde con cualquier pregunta.`
    : `Hi ${c.full_name}, your interview with BSM Facility Solutions is scheduled for ${when}. Reply here with any questions.`
  ...
  if (key && c.email) {
    ...
    const html = es
      ? `<p>Hola ${c.full_name},</p><p>Tu entrevista con <strong>BSM Facility Solutions</strong> está programada para <strong>${when}</strong>.</p><p>Si necesitas reprogramar, responde a este correo.</p><p>— Equipo de Reclutamiento de BSM</p>`
      : `<p>Hi ${c.full_name},</p><p>Your interview with <strong>BSM Facility Solutions</strong> is scheduled for <strong>${when}</strong>.</p><p>If you need to reschedule, just reply to this email.</p><p>— BSM Recruiting Team</p>`
    try {
