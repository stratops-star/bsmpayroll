...
const URG_CLS: Record<string, string> = { immediate: 'bg-red-100 text-red-700', high: 'bg-amber-100 text-amber-700', standard: 'bg-[var(--raise)] text-[var(--text)]' }
  ...
  return (
    <div className="min-h-screen bsm-app">
      <div className="max-w-6xl mx-auto px-6 py-5">
        <div className="mb-4"><h1 className="text-xl font-semibold text-[var(--text-strong)]">{t('tab_requests')}</h1><p className="text-xs text-[var(--muted)]">{t('requests_sub')}</p></div>
        <RecruitingTabs />
        {loading ? <p className="text-[var(--faint)] text-sm">{t('loading')}</p>
          : reqs.length === 0 ? <div className="bg-[var(--surface)] border border-[var(--border)] rounded-xl p-10 text-center text-[var(--muted)]">{t('no_requests_yet')}</div>
          : DEPT_ORDER.filter(d => grouped[d]?.length).map(d => (
            <div key={d} className="mb-6">
              <div className="flex items-center gap-2 mb-2.5"><h2 className="text-sm font-bold text-[var(--text)] uppercase tracking-wide">{DEPT_LABEL[d]}</h2><span className="text-xs text-[var(--faint)]">{grouped[d].length}</span></div>
              <div className="grid gap-3" style={{ gridTemplateColumns: 'repeat(auto-fill,minmax(320px,1fr))' }}>
                {grouped[d].map(r => {
                  ...
                  return (
                    <button key={r.id} onClick={() => setSel(r)} className="text-left bg-[var(--surface)] border border-[var(--border)] rounded-xl p-4 hover:shadow-lg hover:-translate-y-0.5 transition-all">
                      <div className="flex items-center justify-between mb-1.5">
                        <span className="text-xs font-bold text-[var(--text)]">#{r.seq}</span>
                        <div className="flex items-center gap-1.5">
                          {r.urgency && <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${URG_CLS[r.urgency] || 'bg-[var(--raise)] text-[var(--text)]'}`}>{t('urgency_' + r.urgency) !== 'urgency_' + r.urgency ? t('urgency_' + r.urgency) : r.urgency}</span>}
                          <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full ${r.status === 'open' ? 'bg-emerald-50 text-emerald-700' : 'bg-[var(--raise)] text-[var(--muted)]'}`}>{r.status === 'open' ? t('status_open') : t('status_filled')}</span>
                        </div>
                      </div>
                      <div className="font-semibold text-[var(--text)] text-[15px]">{genderL} · {transL} · {r.position || '—'}</div>
                      <div className="text-xs text-[var(--muted)] mt-0.5">{mgr}{r.site ? ` · ${r.site}` : ''}</div>
                      <div className="mt-3 space-y-1.5">
                        <div className="flex items-center gap-2"><span className="text-[10px] text-[var(--faint)] w-16 flex-shrink-0">{t('bar_selected')}</span><div className="flex-1 h-1.5 bg-[var(--raise)] rounded-full overflow-hidden"><div className="h-full bg-[var(--gold)]" style={{ width: `${Math.min(100, (selected / need) * 100)}%` }} /></div><span className="text-[11px] text-[var(--muted)] font-semibold w-8 text-right">{selected}/{need}</span></div>
                        <div className="flex items-center gap-2"><span className="text-[10px] text-[var(--faint)] w-16 flex-shrink-0">{t('bar_assigned')}</span><div className="flex-1 h-1.5 bg-[var(--raise)] rounded-full overflow-hidden"><div className="h-full bg-black/60" style={{ width: `${Math.min(100, (options / CAP) * 100)}%` }} /></div><span className="text-[11px] text-[var(--muted)] w-8 text-right">{options}/{CAP}</span></div>
                      </div>
                    </button>
                  )
                })}
        ...
        <>
          ...
          <aside className="fixed top-0 right-0 h-screen w-[460px] max-w-[96vw] bg-[var(--surface)] z-30 shadow-2xl flex flex-col">
            <div className="p-5 border-b border-[var(--border)] relative">
              ...
              <div className="text-xs font-bold text-[var(--text)]">#{sel.seq} · {DEPT_LABEL[sel.department || 'other']}</div>
              <h2 className="text-lg font-semibold text-[var(--text-strong)] mt-0.5">{sel.position}</h2>
              <div className="text-xs text-[var(--muted)]">{sel.supervisor_name}</div>
            ...
            <div className="overflow-auto flex-1">
              <div className="p-5 border-b border-[var(--border)] grid grid-cols-2 gap-y-2 text-[13px]">
              ...
              {sel.expectation_details && <div className="p-5 border-b border-[var(--border)]"><div className="text-[11px] uppercase tracking-wide text-[var(--text)] font-bold mb-1.5">{t('expectations_l')}</div><p className="text-xs text-[var(--muted)] leading-relaxed whitespace-pre-line">{sel.expectation_details}</p></div>}

              <div className="p-5 border-b border-[var(--border)]">
                <div className="text-[11px] uppercase tracking-wide text-[var(--text)] font-bold mb-2.5">{t('assigned_candidates')} · {assignsFor(sel.id).length}/{neededFor(sel.id) || 1}</div>
                {assignsFor(sel.id).length === 0 ? <p className="text-xs text-[var(--faint)]">{t('no_assigned')}</p> : (
                  <div className="space-y-2">{assignsFor(sel.id).map(a => { const c = cands[a.candidate_id]; if (!c) return null; return (
                    <div key={a.id} className="flex items-center gap-3 bg-[var(--raise)] rounded-lg p-2">
                      ...
                      <div className="flex-1 min-w-0"><div className="text-sm font-semibold text-[var(--text)] truncate">{c.full_name}</div><div className="text-xs text-[var(--muted)]">{(c.positions || [])[0] || '—'}{a.status === 'selected' ? ` · ${t('selected_badge')}` : ''}</div></div>
                      {canAct && <button onClick={() => unassign(a)} className="text-xs text-red-500 font-medium">{t('remove')}</button>}
                    </div>
                  ) })}</div>
                )}
                <p className="text-[11px] text-[var(--faint)] mt-3">{t('assign_from_pool')} → {t('tab_pool')}</p>
              </div>

              {canAct && <div className="p-5 flex gap-2">
                {sel.status === 'open'
                  ? <button onClick={() => setStatus(sel, 'filled')} className="flex-1 bg-[var(--gold)] text-[var(--on-gold)] font-semibold py-2.5 rounded-lg">✓ {t('close_request')}</button>
                  : <button onClick={() => setStatus(sel, 'open')} className="flex-1 bg-[var(--surface)] border border-[var(--border)] text-[var(--muted)] font-semibold py-2.5 rounded-lg">↩ {t('reopen_request')}</button>}
              </div>}
            </div>
          </aside>
        </>
...
function D({ k, v }: { k: string; v: string | null | undefined }) { return <div><div className="text-[11px] text-[var(--faint)]">{k}</div><div className="text-[var(--text)] font-medium capitalize">{v || '—'}</div></div> }
