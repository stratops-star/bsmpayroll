...
type Candidate = {
  ...
  positions: string[] | null; borough: string | null; city: string | null
  rejected_reason: string | null; profile_tier: string | null
}
...
export default function RejectedPage() {
  ...
  async function restore(c: Candidate) {
    const { error } = await supabase.from('candidates').update({ status: 'applied', stage: 'applied' }).eq('id', c.id)
    if (error) { flash('Error: ' + error.message); return }
    setRows(rs => rs.filter(r => r.id !== c.id)); flash('Restored to New Queue')
  }
    ...
    <div className="min-h-screen bg-[#F5F6FA]">
      <div className="bg-[#0D1B35] text-white px-6 py-4 border-b-[3px] border-[#D4A843]">
        ...
        <div className="max-w-5xl mx-auto flex gap-1 mt-3">
          ...
          <a href="/recruiting/rejected" className="text-sm px-3 py-1.5 rounded-lg bg-white/15 text-white font-medium">Rejected</a>
        </div>
      </div>
              ...
              <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
                <table className="w-full text-sm">
                  <thead><tr className="text-left text-[11px] uppercase tracking-wide text-gray-500 bg-gray-50 border-b border-gray-200">
                    <th className="px-4 py-3">Candidate</th><th className="px-4 py-3">Position</th><th className="px-4 py-3">Reason</th><th className="px-4 py-3">Rejected</th>{canAct && <th className="px-4 py-3"></th>}
                  </tr></thead>
                  <tbody>
                    {rows.map(c => (
                      <tr key={c.id} className="border-b border-gray-100 last:border-0">
                        <td className="px-4 py-3"><div className="flex items-center gap-3">
                          <span className="w-9 h-9 rounded-full grid place-items-center text-white text-xs font-semibold opacity-70" style={{ background: hue(c.email || c.full_name) }}>{ini(c.full_name)}</span>
                          <div><div className="font-semibold text-gray-700">{c.full_name}</div><div className="text-xs text-gray-400">{[c.borough, c.city].filter(Boolean).join(', ')}</div></div>
                        </div></td>
