...
export default function NewQueuePage() {
  ...
  useEffect(() => {
    setActions(
      <>
        {isAdmin && <a href="/recruiting/import" className="text-sm bg-[#26221D] text-[#EDE7DD]/10 hover:bg-[#211E1A]/20 text-white border border-white/15 font-medium rounded-lg px-3 py-1.5 whitespace-nowrap">⭳ {t('import')}</a>}
        {canAct && <button onClick={() => setShowAdd(true)} className="text-sm bg-[#DCB878] text-[#1E1B17] font-semibold rounded-lg px-3 py-1.5 whitespace-nowrap">+ {t('add_candidate')}</button>}
        <ShareCareers />
      </>
    )
  ...
  const Field = ({ k, label, opts }: { k: string; label: string; opts: [string, string][] }) => (
    <div><label className="block text-[11px] uppercase tracking-wide text-[#8C8375] font-bold mb-1">{label}</label>
      <select value={F[k]} onChange={e => setF1(k, e.target.value)} className="w-full border border-[#3A342B] rounded-lg px-2.5 py-2 text-sm bg-[#26221D] text-[#EDE7DD]">{opts.map(([v, l]) => <option key={v} value={v}>{l}</option>)}</select></div>
  )

  return (
    <div className="min-h-screen bg-[#1E1B17]">
      <div className="max-w-5xl mx-auto px-6 py-5">
        <div className="mb-4"><h1 className="text-xl font-semibold text-[#EDE7DD]">{t('tab_queue')}</h1><p className="text-xs text-[#8C8375]">{t('queue_sub')}</p></div>

        {/* filter bar */}
        <div className="flex items-center gap-3 mb-3 flex-wrap">
          <input value={q} onChange={e => setQ(e.target.value)} placeholder={t('search_ph')} className="border border-[#3A342B] bg-[#26221D] text-[#EDE7DD] placeholder:text-[#6E665C] rounded-lg px-3 py-2 text-sm w-72 max-w-full" />
          <button onClick={() => setPanelOpen(o => !o)} className="flex items-center gap-2 border border-[#3A342B] bg-[#211E1A] rounded-lg px-3.5 py-2 text-sm font-semibold text-[#EDE7DD]">⛃ {t('filters')} {activeCount > 0 && <span className="bg-[#DCB878] text-[#1E1B17] text-[11px] font-bold rounded-full px-1.5">{activeCount}</span>}</button>
          {(activeCount > 0 || q) && <button onClick={clearAll} className="text-xs text-[#8C8375] underline">{t('clear_filters')}</button>}
        ...
        {panelOpen && (
          <div className="bg-[#211E1A] border border-[#3A342B] rounded-xl p-4 mb-4">
            <div className="grid gap-3" style={{ gridTemplateColumns: 'repeat(auto-fit,minmax(170px,1fr))' }}>
              ...
              <div><label className="block text-[11px] uppercase tracking-wide text-[#8C8375] font-bold mb-1">{t('f_from')}</label><input type="date" style={{colorScheme:"dark"}} value={from} onChange={e => { setFrom(e.target.value); setF1('date', 'all') }} className="w-full border border-[#3A342B] bg-[#26221D] text-[#EDE7DD] rounded-lg px-2.5 py-2 text-sm" /></div>
              <div><label className="block text-[11px] uppercase tracking-wide text-[#8C8375] font-bold mb-1">{t('f_to')}</label><input type="date" style={{colorScheme:"dark"}} value={to} onChange={e => { setTo(e.target.value); setF1('date', 'all') }} className="w-full border border-[#3A342B] bg-[#26221D] text-[#EDE7DD] rounded-lg px-2.5 py-2 text-sm" /></div>
            </div>
            <div className="flex justify-end mt-3"><button onClick={clearAll} className="text-xs text-[#8C8375] underline">{t('clear_filters')}</button></div>
          </div>
        )}

        <RecruitingTabs newCount={newCount} />

        {loading ? <p className="text-[#6E665C] text-sm">{t('loading')}</p>
          : rows.length === 0 ? <div className="bg-[#211E1A] border border-[#3A342B] rounded-xl p-10 text-center text-[#8C8375]">{t('no_new_apps')}</div>
          : (<>
            <div className="flex items-center gap-2 mb-4">
              <span className="font-semibold text-[#EDE7DD]">{filtered.length}{filtered.length !== rows.length ? ` of ${rows.length}` : ''} application{rows.length > 1 ? 's' : ''}</span>
              {newCount > 0 && <span className="text-xs text-[#DCB878] bg-[#2A241D] px-2 py-0.5 rounded-full">{newCount} arrived just now</span>}
            </div>
            {filtered.length === 0 ? <div className="bg-[#211E1A] border border-[#3A342B] rounded-xl p-10 text-center text-[#8C8375]">No applications match these filters.</div>
              : <div className="grid gap-3" style={{ gridTemplateColumns: 'repeat(auto-fill,minmax(300px,1fr))' }}>
                {filtered.map(c => (
                  <button key={c.id} onClick={() => open(c)} className="text-left bg-[#211E1A] border border-[#3A342B] border-l-[3px] border-l-[#DCB878] rounded-xl p-4 hover:shadow-lg hover:-translate-y-0.5 transition-all">
                    <div className="flex items-center gap-3 mb-3">
                      <Avatar name={c.full_name} src={photos[c.id]} size={40} />
                      <div><div className="font-semibold text-[#EDE7DD] flex items-center gap-2">{c.full_name}<SourceBadge channel={c.intake_channel} /></div><div className="text-xs text-[#8C8375]">{[c.borough, c.city, c.state].filter(Boolean).join(', ') || '—'}</div></div>
                    </div>
                    <div className="flex flex-wrap gap-1.5 mb-3">{(c.positions || []).slice(0, 3).map(p => <span key={p} className="text-[11px] bg-[#2A241D] text-[#EDE7DD] px-2 py-0.5 rounded-full">{p}</span>)}</div>
                    <div className="flex items-center justify-between text-[11px] text-[#6E665C] border-t border-[#3A342B] pt-2.5"><span>{c.preferred_lang === 'es' ? '🇪🇸' : '🇺🇸'} · {payText(c)} · {fmtDate(c.created_at)}</span><span>{ago(c.created_at)}</span></div>
                  </button>
                ))}
              </div>}
          </>)}
      ...
      {sel && (
        <>
          ...
          <aside className="fixed top-0 right-0 h-screen w-[440px] max-w-[94vw] bg-[#211E1A] z-30 shadow-2xl flex flex-col">
            <div className="p-5 border-b border-[#3A342B] relative">
              <button onClick={close} className="absolute top-4 right-5 w-8 h-8 rounded-lg bg-[#2A241D] text-[#8C8375]">✕</button>
              <div className="flex items-center gap-3"><Avatar name={sel.full_name} src={photos[sel.id]} size={48} />
                <div><h2 className="text-lg font-semibold text-[#EDE7DD]">{sel.full_name}</h2><div className="text-xs text-[#8C8375] flex items-center gap-2">{ago(sel.created_at)} · {sel.preferred_lang === 'es' ? 'Español' : 'English'}<SourceBadge channel={sel.intake_channel} /></div></div></div>
            </div>
            <div className="overflow-auto flex-1">
              {canAct ? (
                <Sec title={t('initial_review')}>
                  <div className="text-xs text-[#8C8375] mb-1.5">{t('profile_tier')}</div>
                  <div className="flex gap-2 mb-3">{['high', 'medium', 'low'].map(tk => <button key={tk} onClick={() => setTier(tk)} className={`text-xs font-semibold px-3 py-1.5 rounded-full border capitalize ${tier === tk ? 'bg-[#DCB878] border-[#DCB878] text-[#1E1B17]' : 'border-[#3A342B] text-[#8C8375]'}`}>{tk}</button>)}</div>
                  <div className="flex gap-2">
                    <button disabled={busy} onClick={toInterview} className="flex-1 bg-[#DCB878] text-[#1E1B17] text-sm font-semibold py-2.5 rounded-lg disabled:opacity-50">🎥 {t('send_to_virtual')}</button>
                  ...
                  <input value={reason} onChange={e => setReason(e.target.value)} placeholder={t('reject_reason_ph')} className="w-full mt-2 border border-[#3A342B] bg-[#26221D] text-[#EDE7DD] placeholder:text-[#6E665C] rounded-lg px-3 py-2 text-xs" />
                  <button disabled={busy} onClick={openOverride} className="w-full mt-2 bg-transparent border border-[#5C4A2A] text-[#DCB878] text-xs font-semibold py-2 rounded-lg disabled:opacity-50">⏭ {t('override')}</button>
                </Sec>
              ) : <Sec title={t('view_only')}><p className="text-xs text-[#8C8375]">{t('view_only_msg')}</p></Sec>}
              <Sec title={t('s_applied_for')}>
                <div className="flex flex-wrap gap-1.5">{(sel.positions || []).map(p => <span key={p} className="text-xs bg-[#2A241D] text-[#DCB878] font-medium px-2.5 py-1 rounded-full">{p}</span>)}</div>
              ...
              {sel.experience && <Sec title={t('s_experience')}><p className="text-xs text-[#8C8375] leading-relaxed bg-[#2A241D] rounded-lg p-3 whitespace-pre-line">{sel.experience}</p></Sec>}
              {sel.strengths && <Sec title="Strengths"><p className="text-xs text-[#8C8375] leading-relaxed">{sel.strengths}</p></Sec>}
              <Sec title={t('s_source_files')}><Rw k={t('l_heard_via')} v={sel.referral_source} />{sel.resume_path ? <button onClick={() => openFile('candidate-resumes', sel.resume_path)} className="mt-1 inline-flex items-center gap-2 bg-[#2A241D] border border-[#3A342B] rounded-lg px-3 py-2 text-sm font-medium text-[#EDE7DD]"><span className="bg-red-500 text-white text-[9px] font-bold px-1.5 py-0.5 rounded">PDF</span> {t('view_resume')}</button> : <div className="text-xs text-[#6E665C] mt-1">{t('no_resume')}</div>}</Sec>
            </div>
          </aside>
        </>
      )}
      ...
      {overrideOpen && (
        <div className="fixed inset-0 bg-black/40 z-[60] flex items-center justify-center p-4" onClick={() => setOverrideOpen(false)}>
          <div className="bg-[#211E1A] rounded-2xl w-full max-w-md p-6" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-2"><h2 className="text-lg font-semibold text-[#EDE7DD]">{t('override_title')}</h2><button onClick={() => setOverrideOpen(false)} className="w-8 h-8 rounded-lg bg-[#2A241D] text-[#8C8375]">✕</button></div>
            <p className="text-xs text-[#8C8375] mb-4">{t('override_desc')}</p>
            {requests.length === 0 ? (
              <div className="text-sm text-[#DCB878] bg-[#2A241D] border border-[#3A342B] rounded-lg px-3 py-3">{t('no_open_requests')}</div>
            ) : (<>
              <label className="text-xs font-semibold text-[#8C8375]">{t('pick_request')}</label>
              <select value={reqId} onChange={e => setReqId(e.target.value)} className="w-full mt-1 mb-4 border border-[#3A342B] rounded-lg px-3 py-2 text-sm bg-[#26221D] text-[#EDE7DD]">
                <option value="">—</option>
                {requests.map(r => <option key={r.id} value={r.id}>{r.label}</option>)}
              </select>
              <button disabled={busy || !reqId} onClick={doOverride} className="w-full bg-[#DCB878] text-[#1E1B17] font-semibold py-2.5 rounded-lg disabled:opacity-40">⏭ {t('confirm_override')}</button>
            </>)}
          </div>
        </div>
      )}
      {toast && <div className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-[#211E1A] border border-[#3A342B] text-[#EDE7DD] px-5 py-3 rounded-xl text-sm font-medium shadow-xl z-40"><span className="text-[#DCB878]">✓</span> {toast}</div>}
    </div>
  )
}
...
function AddCandidate({ supabase, onClose, onAdded }: { supabase: any; onClose: () => void; onAdded: (c: Candidate) => void }) {
  ...
  const input = 'w-full border border-[#3A342B] bg-[#26221D] text-[#EDE7DD] placeholder:text-[#6E665C] rounded-lg px-3 py-2 text-sm'
  const pill = (on: boolean) => `text-xs font-medium px-3 py-1.5 rounded-full border ${on ? 'bg-[#DCB878] text-[#1E1B17] border-[#DCB878]' : 'bg-transparent text-[#8C8375] border-[#3A342B]'}`
  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-[#211E1A] rounded-2xl w-full max-w-lg max-h-[90vh] overflow-auto p-6" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-[#EDE7DD]">{L(TR.addCandidate)}</h2>
          <div className="flex items-center gap-2">
            <div className="flex gap-0.5 bg-[#2A241D] rounded-lg p-0.5">{(['en', 'es'] as const).map(l => <button key={l} onClick={() => setLang(l)} className={`text-xs font-semibold px-2 py-1 rounded-md ${lang === l ? 'bg-[#DCB878] text-[#1E1B17]' : 'text-[#8C8375]'}`}>{l.toUpperCase()}</button>)}</div>
            <button onClick={onClose} className="w-8 h-8 rounded-lg bg-[#2A241D] text-[#8C8375]">✕</button>
          </div>
        </div>
        <div className="space-y-3">
          ...
          <div><div className="text-xs font-semibold text-[#8C8375] mb-1.5">{L(TR.positions)} {star}</div><div className="flex flex-wrap gap-1.5 max-h-32 overflow-auto p-1">{MANUAL_POS.map(p => <button key={p} type="button" onClick={() => toggle(positions, setPositions, p)} className={pill(positions.includes(p))}>{p}</button>)}</div></div>
          {positions.includes('Security') && <div><div className="text-xs font-semibold text-[#8C8375] mb-1.5">Security license {star}</div><div className="flex gap-2"><button type="button" onClick={() => setLic('yes')} className={pill(lic === 'yes')}>Licensed</button><button type="button" onClick={() => setLic('no')} className={pill(lic === 'no')}>Unlicensed</button></div></div>}
          ...
          <div><div className="text-xs font-semibold text-[#8C8375] mb-1.5">{L(TR.transportation)} {star}</div><div className="flex flex-wrap gap-1.5">{TRANSPORT.map(t => <button key={t} type="button" onClick={() => toggle(trans, setTrans, t)} className={pill(trans.includes(t))}>{t}</button>)}</div></div>
          ...
          <button disabled={busy} onClick={submit} className="w-full bg-[#DCB878] text-[#1E1B17] font-semibold py-2.5 rounded-lg disabled:opacity-50">{busy ? '…' : L(TR.addToQueue)}</button>
        </div>
      </div>
    </div>
  )
...
function Sec({ title, children }: { title: string; children: React.ReactNode }) { return <div className="p-5 border-b border-[#3A342B]"><div className="text-[11px] uppercase tracking-wide text-[#EDE7DD] font-bold mb-2.5">{title}</div>{children}</div> }
function Rw({ k, v }: { k: string; v: string | null | undefined }) { return <div className="flex justify-between gap-3 py-1 text-[13px]"><span className="text-[#8C8375]">{k}</span><span className="text-[#EDE7DD] font-medium text-right">{v || '—'}</span></div> }
