...
export default function ValetManager() {
  ...
  const load = useCallback(async () => {
    ...
    const { data } = await supabase
      .from('app_users').select('id, full_name, email, phone, active')
      .eq('role', 'valet').order('full_name')
  ...
  async function addAttendant() {
    ...
    setF({ full_name: '', email: '', phone: '' }); setAdding(false)
    load()
  }
  ...
  async function toggle(a: Attendant) {
    ...
    if (r.error) { flash(r.error); return }
    flash(a.active ? 'Deactivated' : 'Reactivated')
    load()
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F1F3F8', fontFamily: 'system-ui, sans-serif' }}>
      ...
      {toast && <div style={{ position: 'fixed', bottom: 20, left: '50%', transform: 'translateX(-50%)', background: NAVY, color: '#fff', padding: '10px 18px', borderRadius: 999, fontSize: 14, zIndex: 50, boxShadow: '0 8px 24px rgba(0,0,0,.3)', maxWidth: '92%', textAlign: 'center' }}>{toast}</div>}
      ...
      <main style={{ maxWidth: 620, margin: '0 auto', padding: 16 }}>
        ...
        {adding && (
          <div style={{ ...card, padding: 16, marginBottom: 16 }}>
            ...
            <p style={{ fontSize: 12, color: '#94A3B8', margin: '2px 0 12px' }}>
              A login + temporary password is created and sent to them by email {f.phone ? '+ text' : ''}.
            </p>
            <div style={{ display: 'flex', gap: 8 }}>
        ...
        <p style={{ fontSize: 12, color: '#94A3B8', textAlign: 'center', marginTop: 18 }}>
          Tenants &amp; units management arrives next.
        </p>
      </main>
    </div>
  )
