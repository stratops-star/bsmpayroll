...
export default function PoolBoard({ standalone = false, canAssign: canAssignProp = true }: { standalone?: boolean; canAssign?: boolean }) {
  ...
  if (standalone && gate !== 'ok') return <div className="min-h-screen grid place-items-center bg-[#F5F6FA] p-6"><div className="bg-white rounded-2xl p-8 max-w-md text-center shadow-xl border-t-4 border-[#D4A843]"><div className="text-4xl mb-3">{gate === 'pending' ? '⏳' : '🔒'}</div><h1 className="text-lg font-bold text-[#0D1B35]">{gate === 'pending' ? 'Access pending approval' : 'Sign in required'}</h1><p className="text-sm text-gray-500 mt-2">{gate === 'pending' ? 'An administrator needs to grant you pool access.' : 'Please sign in to continue.'}</p></div></div>

  return (
    <div className="min-h-screen bg-[#F5F6FA]">
      {standalone && (
        <div className="bg-[#0D1B35] text-white px-6 py-3 border-b-[3px] border-[#D4A843]">
          <div className="max-w-6xl mx-auto flex items-center gap-3">
            <span className="w-8 h-8 rounded-lg grid place-items-center font-bold text-sm bg-[#D4A843] text-[#0D1B35]">B</span>
            ...
            <button onClick={() => setLang(lang === 'es' ? 'en' : 'es')} className="text-xs bg-white/10 border border-white/15 rounded-lg px-2.5 py-1.5 font-medium">{lang === 'es' ? 'EN' : 'ES'}</button>
      ...
      <div className="max-w-6xl mx-auto px-6 py-5">
        <div className="mb-4"><h1 className="text-xl font-semibold text-[#0D1B35]">{t('tab_pool')}</h1><p className="text-xs text-gray-500">{t('pool_sub')}</p></div>
        <div className="flex items-center gap-3 mb-3 flex-wrap">
          ...
          <select value={sort} onChange={e => setSort(e.target.value as any)} className="border border-gray-200 bg-white rounded-lg px-3 py-2 text-sm text-gray-600"><option value="recent">{t('sort_recent')}</option><option value="longest">{t('sort_longest')}</option></select>
          {(() => { const n = rows.filter(c => missingFields(c).length > 0).length; return <button onClick={() => setF1('profile', F.profile === 'missing' ? 'all' : 'missing')} className={`flex items-center gap-1.5 rounded-lg px-3 py-2 text-sm font-semibold border ${F.profile === 'missing' ? 'bg-amber-500 text-white border-amber-500' : 'bg-white text-amber-700 border-amber-200'}`}>⚠ {t('missing_info')}{n > 0 && <span className={`text-[11px] font-bold rounded-full px-1.5 ${F.profile === 'missing' ? 'bg-white/25' : 'bg-amber-100'}`}>{n}</span>}</button> })()}
          <div className="ml-auto flex gap-1 bg-white border border-gray-200 rounded-lg p-0.5">{(['list', 'photos'] as const).map(v => <button key={v} onClick={() => setView(v)} className={`text-xs font-semibold px-3 py-1.5 rounded-md ${view === v ? 'bg-[#0D1B35] text-white' : 'text-gray-500'}`}>{v === 'list' ? `▤ ${t('list')}` : `▦ ${t('photos')}`}</button>)}</div>
        </div>

        {activeCount > 0 && <div className="flex flex-wrap gap-2 mb-3 items-center">{Object.entries(F).filter(([, v]) => v !== 'all').map(([k, v]) => <span key={k} className="bg-[#0D1B35] text-white text-xs px-2.5 py-1 rounded-full flex items-center gap-1.5">{FLABEL[k] || FILTERS.find(f => f.key === k)?.label}: {chipLabel(v)}<button onClick={() => setF1(k, 'all')} className="opacity-70">✕</button></span>)}<button onClick={clearAll} className="text-xs text-gray-500 underline">{t('clear_filters')}</button></div>}

        {panelOpen && (
          <div className="bg-white border border-gray-200 rounded-xl p-4 mb-4">
            <div className="grid gap-3" style={{ gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))' }}>
              {FILTERS.map(f => <div key={f.key}><label className="block text-[11px] uppercase tracking-wide text-gray-500 font-bold mb-1">{FLABEL[f.key] || f.label}</label><select value={F[f.key]} onChange={e => setF1(f.key, e.target.value)} className="w-full border border-gray-200 rounded-lg px-2.5 py-2 text-sm bg-white">{f.opts.map(([v, l]) => <option key={v} value={v}>{l}</option>)}</select></div>)}
            </div>
        ...
        {loading ? <p className="text-gray-400 text-sm">Loading…</p>
          : filtered.length === 0 ? <div className="bg-white border border-gray-100 rounded-xl p-10 text-center text-gray-500">{lang === 'es' ? 'Ningún candidato coincide con estos filtros.' : 'No candidates match these filters.'}</div>
          : view === 'photos' ? (
            ...
            <div className="overflow-auto flex-1">
                ...
                {canAct ? (
                  <div className="space-y-2.5">
                    <div className="grid grid-cols-2 gap-2">
                      <label className="text-xs text-gray-600">{t('l_gender')}{!sel.gender && <Miss text={t('missing')} />}<select defaultValue={sel.gender || ''} onBlur={e => e.target.value !== (sel.gender || '') && save({ gender: e.target.value || null } as any)} className={`w-full mt-1 border rounded-lg px-2 py-1.5 text-sm ${!sel.gender ? 'border-red-200 bg-red-50/40' : 'border-gray-200'}`}><option value=""></option><option value="female">Female</option><option value="male">Male</option><option value="other">Other</option></select></label>
                      <label className="text-xs text-gray-600">{t('l_age')}{sel.age == null && <Miss text={t('missing')} />}<input type="number" defaultValue={sel.age ?? ''} onBlur={e => Number(e.target.value) !== (sel.age ?? NaN) && save({ age: e.target.value ? Number(e.target.value) : null } as any)} className={`w-full mt-1 border rounded-lg px-2 py-1.5 text-sm ${sel.age == null ? 'border-red-200 bg-red-50/40' : 'border-gray-200'}`} /></label>
                      <div className="text-xs text-gray-600">{t('l_nationality')}{!sel.nationality && <Miss text={t('missing')} />}<div className="mt-1"><SearchSelect value={sel.nationality} onChange={v => save({ nationality: v || null } as any)} options={NATIONALITIES} placeholder={t('ph_nationality')} /></div></div>
                      <div className="text-xs text-gray-600">{t('l_ethnicity')}{!sel.ethnicity && <Miss text={t('missing')} />}<div className="mt-1"><SearchSelect value={sel.ethnicity} onChange={v => save({ ethnicity: v || null } as any)} options={ETHNICITIES} placeholder={t('ph_ethnicity')} /></div></div>
                      <div className="text-xs text-gray-600 col-span-2">{t('l_time_usa')}{!sel.time_in_usa && <Miss text={t('missing')} />}<div className="mt-1"><YearsMonths value={sel.time_in_usa} onSave={v => save({ time_in_usa: v || null } as any)} yearsLabel={t('years')} monthsLabel={t('months')} /></div></div>
                    </div>
                    <div className="grid grid-cols-3 gap-2 pt-1">
                      {([[t('l_taxid'), 'has_tax_id'], [t('l_ss'), 'has_ss'], [t('l_bank'), 'has_bank_account']] as [string, keyof Candidate][]).map(([label, key]) => (
                        <div key={key}><div className="text-xs text-gray-600 mb-1">{label}{sel[key] == null && <Miss text={t('missing')} />}</div><div className="flex gap-1.5">
                          {[[t('tax_yes'), true], [t('tax_no'), false]].map(([l, v]) => <button key={l as string} onClick={() => save({ [key]: v } as any)} className={`text-xs font-semibold px-2.5 py-1 rounded-full border ${sel[key] === v ? 'bg-[#0D1B35] text-white border-[#0D1B35]' : 'border-gray-200 text-gray-500'}`}>{l}</button>)}
                        </div></div>
                      ))}
                    </div>
                  </div>
              ...
              <Sec title={t('s_applied_for')}>
                <div className="flex items-center justify-between mb-1.5">
                  <div className="flex flex-wrap gap-1.5">{(sel.positions || []).length ? (sel.positions || []).map(p => <span key={p} className="text-xs bg-[#0D1B35]/5 text-[#0D1B35] font-medium px-2.5 py-1 rounded-full">{p}</span>) : <span className="text-xs text-gray-400">{t('none')}</span>}</div>
                ...
                {editPos && canAct && (
                  <div className="mt-2 bg-[#F5F6FA] rounded-lg p-2.5">
                    <div className="text-[11px] text-gray-500 mb-1.5">{lang === 'es' ? 'Actual — toca ✕ para quitar:' : 'Current — tap ✕ to remove:'}</div>
                    <div className="flex flex-wrap gap-1.5 mb-2.5">
                      {(sel.positions || []).length ? (sel.positions || []).map(p => <button key={p} onClick={() => togglePos(p)} className="text-[11px] font-medium px-2.5 py-1 rounded-full bg-[#0D1B35] text-white flex items-center gap-1">{p} <span className="opacity-70">✕</span></button>) : <span className="text-[11px] text-gray-400">{t('none')}</span>}
                    </div>
                    <div className="text-[11px] text-gray-500 mb-1.5">{lang === 'es' ? 'Agregar un puesto:' : 'Add a position:'}</div>
              ...
              {sel.strengths && <Sec title={t('l_strengths')}><p className="text-xs text-gray-600 leading-relaxed">{sel.strengths}</p></Sec>}
              {canAct && (
                <div className="p-5 border-b border-gray-100">
                  <div className="text-[11px] uppercase tracking-wide text-[#0D1B35] font-bold mb-2.5">{t('l_photo_video')}</div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      {media.photo ? <img src={media.photo} alt="" className="w-full h-32 object-cover rounded-lg mb-1.5" /> : <div className="w-full h-32 bg-[#F5F6FA] rounded-lg grid place-items-center text-gray-400 text-xs mb-1.5">{t('no_photo')}</div>}
                      <label className="text-xs text-[#0D1B35] font-medium cursor-pointer">{media.photo ? t('replace_photo') : t('add_photo')}<input type="file" accept="image/*" className="hidden" onChange={e => uploadMedia('photo', e.target.files?.[0] || null)} /></label>
                    </div>
                    <div>
                      {media.video ? <video src={media.video} controls className="w-full h-32 object-cover rounded-lg mb-1.5 bg-black" /> : <div className="w-full h-32 bg-[#F5F6FA] rounded-lg grid place-items-center text-gray-400 text-xs mb-1.5">{t('no_video')}</div>}
                      <label className="text-xs text-[#0D1B35] font-medium cursor-pointer">{media.video ? t('replace_video') : t('add_video')}<input type="file" accept="video/*" className="hidden" onChange={e => uploadMedia('video', e.target.files?.[0] || null)} /></label>
                    </div>
                  </div>
                </div>
              )}
              <Sec title={t('l_resume_source')}>
                ...
                <div className="flex items-center gap-3 mt-1.5">
                  {media.resume ? <a href={media.resume} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 bg-[#F5F6FA] border border-gray-200 rounded-lg px-3 py-2 text-sm font-medium text-[#0D1B35]"><span className="bg-red-500 text-white text-[9px] font-bold px-1.5 py-0.5 rounded">PDF</span> {t('view_resume')}</a> : <span className="text-xs text-gray-400">{t('no_resume')}</span>}
...
function EField({ label, value, onSave, flag, ml }: { label: string; value: string | null | undefined; onSave: (v: string) => void; flag?: boolean; ml?: string }) {
  ...
  return <div className="py-1"><div className="text-[11px] text-gray-500 mb-0.5">{label}{flag && empty && <Miss text={ml} />}</div><input defaultValue={value || ''} onBlur={e => e.target.value !== (value || '') && onSave(e.target.value)} className={`w-full border rounded-lg px-2.5 py-1.5 text-sm ${flag && empty ? 'border-red-200 bg-red-50/40' : 'border-gray-200'}`} /></div>
}
function ESelect({ label, value, opts, onSave, flag, ml }: { label: string; value: string | null | undefined; opts: string[]; onSave: (v: string) => void; flag?: boolean; ml?: string }) {
  ...
  return <div className="py-1"><div className="text-[11px] text-gray-500 mb-0.5">{label}{flag && empty && <Miss text={ml} />}</div><select defaultValue={valid.includes(value || '') ? (value || '') : ''} onChange={e => onSave(e.target.value)} className={`w-full border rounded-lg px-2.5 py-1.5 text-sm ${flag && empty ? 'border-red-200 bg-red-50/40' : 'border-gray-200'} bg-white`}>{opts.map(o => <option key={o} value={o}>{o || '—'}</option>)}</select></div>
}
