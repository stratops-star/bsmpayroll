// Shared valet report sender — used by the user-facing route and the cron sweep.
...
export type SendResult = { emailed: boolean; reason: string | null; retryable: boolean; attempts: number }

/** Build and email the report for one event. `svc` must be a service-role client. */
export async function sendReportFor(svc: any, event_id: string): Promise<SendResult> {
  const { data: evRow } = await svc.from('valet_events').select(EV_SELECT).eq('id', event_id).maybeSingle()
  if (!evRow) return { emailed: false, reason: 'Event not found.', retryable: false, attempts: 0 }
  const ev = evRow as Ev

  const tenantEmail = ev.valet_customers?.email || ''
  if (!tenantEmail) {
    await svc.from('valet_events').update({ email_error: 'No email on file for this customer.', email_retryable: false }).eq('id', event_id)
    return { emailed: false, reason: 'No email on file for this customer.', retryable: false, attempts: (ev.email_attempts || 0) }
  }
  ...
  try {
    ...
    await svc.from('valet_events').update({ email_error: 'Report PDF could not be generated: ' + (e?.message || 'error'), email_retryable: true, email_attempts: ((ev.email_attempts || 0) + 1) }).eq('id', event_id)
    return { emailed: false, reason: 'Report PDF could not be generated: ' + (e?.message || 'error'), retryable: true, attempts: (ev.email_attempts || 0) + 1 }
  }
  ...
  const unit = base.valet_customers?.valet_units?.unit_number || ''
  const isReturn = ev.action === 'retrieve'
...
<html><body style="margin:0;padding:0;background:#F5F3EF;">
  ...
  const attempts = (ev.email_attempts || 0) + 1
  ...
  return { emailed, reason: emailed ? null : (failReason || 'The report email did not send.'), retryable, attempts }
}
